...
M = sketch_generator.create_hcms_sketch(ldp_data) # Constructing the sketch matrix
...
sns.distplot(data, bins=bins, color='g', ax=axs[0], hist_kws={'ec': "black"})
axs[0].set_title("Hadamard CMS \n Integer data sampled from a Normal distribution \n $N=${}, Sampled from $N({}, {})$".format(N, mu, sd*sd))
# Plotting a distplot of the data produced from the CMS algorithm
sns.distplot(ldp_plot_data, bins=bins, ax=axs[1], color='y', hist_kws={'ec': "black"})
axs[1].set_title("LDP HCMS data produced from the normal sample \n $\epsilon=${}, $m=${}, $k=${}".format(epsilon, m, k))
