...
#This file is a modified version of the rappor.py file in the repo
    # ("https://github.com/google/rappor/blob/master/client/python/rappor.py")

#The main modificiations is that this is a Python 3 port of the Python 2 rappor.py code.
    # The main changes are the way that the code deals with the bytes datatype (introduced in Python 3)
...
class Params(object):
  ...
  """
    ...
    def __init__(self, num_bloombits=16, num_hashes=2, num_of_cohorts=64, probabilities=None):
        if probabilities is None:
            probabilities = [0.50, 0.75, 0.50]

        self.num_bloombits = num_bloombits  # Number of bloom filter bits (k)
        self.num_hashes = num_hashes  # Number of bloom filter hashes (h)
        self.num_cohorts = num_of_cohorts # Number of cohorts (m)

        self.prob_p = probabilities[0]  # Probability p
        self.prob_q = probabilities[1]  # Probability q
        self.prob_f = probabilities[2]  # Probability f
...
def get_bloom_bits(word, cohort, num_hashes, num_bloombits):
  ...
  """
    value = BitArray(auto=to_big_endian(cohort))
    value.append(word)
    value = value.tobytes()
